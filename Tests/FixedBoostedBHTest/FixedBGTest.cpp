...
int main(int argc, char* argv[])
{
    ...
    for (int ires = 0; ires < num_resolutions; ires++)
    {
        ...
        //const int N_GRID = 256 * pow(2, ires);
        const int N_GRID = std::stod(argv[1]) * pow(2, ires);
        ...
        bg_params.mass = 1.0;
        bg_params.velocity = std::stod(argv[3]);
        bg_params.spin = std::stod(argv[4]);
        bg_params.center = center_vector;
        BoostedIsotropicKerrFixedBG boosted_bh(bg_params, dx);
        // BoostedKerrSchildFixedBG boosted_bh(bg_params, dx);
        BoxLoops::loop(
            AssignFixedBGtoBSSNVars<BoostedIsotropicKerrFixedBG>(
                boosted_bh, dx,
        ...
        FixedBGEvolution<FixedBGScalarField<Potential>, BoostedIsotropicKerrFixedBG>
        ...
        // values
        BoxLoops::loop(
            ExcisionTest<FixedBGScalarField<Potential>, BoostedIsotropicKerrFixedBG>(
        ...
        if (/*ires == 0 &&*/ debug_plots_on)
        {
            ...
            filename.append(std::to_string(ires));
            filename.append(".txt");
            ...
            for (bit.begin(); bit.ok(); ++bit)
            {
                ...
                if (iv[1] == N_GRID / 2)
                {
                    ...
                    double out1 = rhs_fab(iv,c_A11);
                    double out2 = rhs_fab(iv,c_A22);
                    double out3 = fixedbg_fab(iv,c_lapse);

                    outfile << std::setw(20) << x << std::setw(20) << z;
                    outfile << std::setw(20) << out1 << std::setw(20) << out2;
                    outfile << std::setw(20) << out3;
                    outfile << "\n";
                }
            }
            outfile.close();
    ...
    for (int i = 0; i < NUM_VARS; ++i)
    {
        for (int ires = 0; ires < num_resolutions-1; ires++)
        {
            ...
            if (abs(hi_res_norm) < 1e-15 && abs(lo_res_norm) < 1e-15)
            {
                lo_res_norm = 1e-8;
                hi_res_norm = 1e-10;
            }
