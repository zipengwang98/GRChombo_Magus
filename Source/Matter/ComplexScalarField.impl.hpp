    ...
    const Tensor<2, data_t> &h_UU, const Tensor<3, data_t> &chris_ULL) const
{
    ...
    FOR2(i, j)
    {
        out.Sij[i][j] = -0.5 * vars.h[i][j] / vars.chi * Vt +
    ...
    FOR2(i, j) { out.Sij[i][j] += -vars.h[i][j] / vars.chi * V_of_phi; }
    ...
    const vars_t<data_t> &advec) const
{
    ...
    total_rhs.Pi_Re =
        vars.lapse * vars.K * vars.Pi_Re + advec.Pi_Re;
    total_rhs.phi_Im = vars.lapse * vars.Pi_Im + advec.phi_Im;
    total_rhs.Pi_Im =
        vars.lapse * vars.K * vars.Pi_Im + advec.Pi_Im;
    ...
    FOR(i, j)
    {
        ...
        total_rhs.Pi_Re += h_UU[i][j] * (-0.5 * d1.chi[j] * vars.lapse * d1.phi_Re[i] +
                                vars.chi * vars.lapse * d2.phi_Re[i][j] +
                                vars.chi * d1.lapse[i] * d1.phi_Re[j]);
        total_rhs.Pi_Im += h_UU[i][j] * (-0.5 * d1.chi[j] * vars.lapse * d1.phi_Im[i] +
                                vars.chi * vars.lapse * d2.phi_Im[i][j] +
                                vars.chi * d1.lapse[i] * d1.phi_Im[j]);
        FOR(k)
        {
            total_rhs.Pi_Re += -vars.chi * vars.lapse * h_UU[i][j] * chris.ULL[k][i][j] *
                      d1.phi_Re[k];
            total_rhs.Pi_Im += -vars.chi * vars.lapse * h_UU[i][j] * chris.ULL[k][i][j] *
                      d1.phi_Im[k];
        }
    }
