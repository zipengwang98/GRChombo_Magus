...
template <class matter_t, class background_t> class FixedBGEnergyAndMomFlux
{
  ...
  public:
    FixedBGEnergyAndMomFlux(matter_t a_matter, background_t a_background, double a_dx,
	...
	FOR2(i, j)
	{
	  si_norm += gamma_UU[i][j] * si_L[i] * si_L[j];
	}
	...
	Mdot *= dArea / r2sintheta;
        ...
        FOR1(i)
        {
            ...
            {
                Edot +=
                    -si_L[i] * emtensor.Si[j] * (shift[i] * shift[j] +
                   lapse * lapse * gamma_UU[i][j]);
                FOR1(k)
                {
                    Edot += si_L[i] * lapse * gamma_UU[i][j] *
                      shift[k] * emtensor.Sij[j][k];
                }
            }
        }
