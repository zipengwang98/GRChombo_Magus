...
#if !defined(STRESSEXTRACTION_HALVES_HPP_)
#error "This file should only be included through StressExtraction_halves.hpp"
#endif

#ifndef STRESSEXTRACTION_HALVES_IMPL_HPP_
#define STRESSEXTRACTION_HALVES_IMPL_HPP_

//! Set up and execute the interpolation query
inline void StressExtraction_halves::execute_query(
    AMRInterpolator<Lagrange<4>> *a_interpolator) const
{
    CH_TIME("StressExtraction_halves::execute_query");
    ...
    for (int imode = 0; imode < m_params.num_modes; ++imode)
    {
        ...
        std::string integral_filename = "Stress_integral_halves_" +
...
StressExtraction_halves::integrate_surface(int es, int el, int em,
                                  const std::vector<double> a_Stress,
				  const std::vector<double> a_dArea) const
{
    CH_TIME("StressExtraction_halves::integrate_surface");
    int rank;
#ifdef CH_MPI
    ...
    rank = 0;
#endif
    std::vector<double> integral_Stress1(m_params.num_extraction_radii, 0.);
    //std::vector<double> inner_integral_Momx(m_params.num_extraction_radii, 0.);
    std::vector<double> integral_Stress2(m_params.num_extraction_radii, 0.);
...
#pragma omp parallel for collapse(2) default(none)                             \
  shared(es, el, em, integral_Stress1, integral_Stress2) OPENMP_CONST_SHARED
...
#pragma omp atomic
#endif
		  integral_Stress1[iradius] += m_dphi * inner_integral_Stress;
		}
	      else
		{
#ifdef _OPENMP
#pragma omp atomic
#endif
		  integral_Stress2[iradius] += m_dphi * inner_integral_Stress;
		}
	    }
        }
    }
    return std::make_pair(integral_Stress1, integral_Stress2);
}

//! Write out calculated value of integral for each extraction radius
inline void
StressExtraction_halves::write_integral(const std::vector<double> a_integral_Stress1,
                               const std::vector<double> a_integral_Stress2,
                               std::string a_filename) const
{
    CH_TIME("StressExtraction_halves::write_integral");
    ...
    if (m_first_step)
    {
             ...
             iintegral += 2)
        {
            ...
            header1_strings[iintegral] = "integral Stress1";
            header1_strings[iintegral1] = "integral Stress2";
         ...
         iintegral += 2)
    {
        int iintegral1 = iintegral + 1;
        int iradius = iintegral / 2;
        data_for_writing[iintegral] = a_integral_Stress1[iradius];
        data_for_writing[iintegral1] = a_integral_Stress2[iradius];
    }
...
inline void
StressExtraction_halves::write_extraction(std::string a_file_prefix,
                                   const std::vector<double> a_Stress,
				   const std::vector<double> a_dArea) const
{
    CH_TIME("StressExtraction_halves::write_extraction");
    ...
    for (int iradius = 0; iradius < m_params.num_extraction_radii; ++iradius)
    {
	  ...
	  int iphi = idx % m_params.num_points_phi;
	      // (idx - iradius * m_num_points) / m_params.num_points_phi; //idx % m_params.num_points_phi;
            // don't put a point at z = 0
            double theta = (itheta + 0.5) * m_dtheta;
            double phi = (iphi + 0.5) * m_dphi;
...
#endif /* STRESSEXTRACTION_HALVES_IMPL_HPP_ */
